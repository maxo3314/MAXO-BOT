...
BOT_TOKEN_HARDCODED = "8893354316:AAHiMsGG2aRXdIvUrc-pt4PNA68qnIKEpRs"     # مثلا: "123456789:AAAbbbCCCdddEEEfffGGG"
ADMIN_IDS_HARDCODED = "8303160469"     # مثلا: "7195511082" یا برای چند ادمین: "7195511082,111222333"
...
def init_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS button_colors(
...
def get_button_color(button_text):
    try:
        conn = get_conn()
        row = conn.execute("SELECT color FROM button_colors WHERE button_text=?", (button_text,)).fetchone()
        conn.close()
...
def set_button_color(button_text, color):
    if color not in ("default", "primary", "success", "danger"):
        color = "default"
    conn = get_conn()
    conn.execute("INSERT INTO button_colors(button_text,color) VALUES(?,?) ON CONFLICT(button_text) DO UPDATE SET color=excluded.color",
                 (button_text, color))
    conn.commit()
    conn.close()


def styled_reply_keyboard(rows):
    styled_rows = []
    for row in rows:
        styled_row = []
        for text in row:
            ...
            if color in ("primary", "success", "danger"):
                styled_row.append(KeyboardButton(text=text, style=color))
            else:
                styled_row.append(KeyboardButton(text=text))
...
def main_menu_kb(user_id):
    ...
    if is_admin(user_id):
        rows.append(["مدیریت 🛠"])
    return styled_reply_keyboard(rows)
...
def admin_menu_kb(user_id):
    ...
    if owner:
        rows.insert(-1, ["🎨 تغییر رنگ دکمه‌ها"])
...
COLOR_BUTTONS = sorted(ALL_MENU_TEXTS)
COLOR_NAMES = {
    "default": "⚪ پیش‌فرض",
    "danger": "🔴 قرمز",
    "success": "🟢 سبز",
...
@owner_only
async def admin_button_color_start(update, context):
    rows = []
    for i, text in enumerate(COLOR_BUTTONS):
        color = get_button_color(text)
        icon = {"default":"⚪", "danger":"🔴", "success":"🟢", "primary":"🔵"}.get(color, "⚪")
        rows.append([InlineKeyboardButton(f"{icon} {text}", callback_data=f"colorpick:{i}")])
    rows.append([InlineKeyboardButton("بستن ❌", callback_data="colorlist_close")])
    await update.message.reply_text("🎨 تغییر رنگ دکمه‌ها\n\nدکمه موردنظر را انتخاب کنید:", reply_markup=InlineKeyboardMarkup(rows))


@owner_only
async def admin_button_color_pick(update, context, index):
    query = update.callback_query
    if index < 0 or index >= len(COLOR_BUTTONS):
        await query.answer("دکمه نامعتبر است.", show_alert=True)
        return
    text = COLOR_BUTTONS[index]
    context.user_data["color_edit_button"] = text
    current = get_button_color(text)
    rows = [
        [InlineKeyboardButton("🔴 قرمز", callback_data="colorset:danger", style="danger"),
         InlineKeyboardButton("🟢 سبز", callback_data="colorset:success", style="success")],
        [InlineKeyboardButton("🔵 آبی", callback_data="colorset:primary", style="primary"),
         InlineKeyboardButton("⚪ پیش‌فرض", callback_data="colorset:default")],
        [InlineKeyboardButton("↩️ برگشت", callback_data="colorlist")],
    ]
    await query.edit_message_text(f"دکمه: {text}\nرنگ فعلی: {COLOR_NAMES.get(current, current)}\n\nرنگ جدید را انتخاب کنید:", reply_markup=InlineKeyboardMarkup(rows))


@owner_only
async def admin_button_color_set(update, context, color):
    query = update.callback_query
    text = context.user_data.get("color_edit_button")
    if not text:
        await query.answer("اول یک دکمه انتخاب کنید.", show_alert=True)
        return
    set_button_color(text, color)
    await query.answer("رنگ ذخیره شد")
    # redraw the list
    rows = []
    for i, label in enumerate(COLOR_BUTTONS):
        c = get_button_color(label)
        icon = {"default":"⚪", "danger":"🔴", "success":"🟢", "primary":"🔵"}.get(c, "⚪")
        rows.append([InlineKeyboardButton(f"{icon} {label}", callback_data=f"colorpick:{i}")])
    rows.append([InlineKeyboardButton("بستن ❌", callback_data="colorlist_close")])
    await query.edit_message_text(f"✅ رنگ «{text}» روی {COLOR_NAMES[color]} تنظیم شد.\n\nیک دکمه دیگر را انتخاب کنید:", reply_markup=InlineKeyboardMarkup(rows))


@owner_only
async def admin_button_color_list(update, context):
    query = update.callback_query
    rows = []
    for i, text in enumerate(COLOR_BUTTONS):
        c = get_button_color(text)
        icon = {"default":"⚪", "danger":"🔴", "success":"🟢", "primary":"🔵"}.get(c, "⚪")
        rows.append([InlineKeyboardButton(f"{icon} {text}", callback_data=f"colorpick:{i}")])
    rows.append([InlineKeyboardButton("بستن ❌", callback_data="colorlist_close")])
    await query.edit_message_text("🎨 تغییر رنگ دکمه‌ها\n\nدکمه موردنظر را انتخاب کنید:", reply_markup=InlineKeyboardMarkup(rows))
...
async def admin_menu_dispatch(update, context, text):
    mapping = {
        ...
        "🎨 تغییر رنگ دکمه‌ها": admin_button_color_start,
    }
...
async def admin_callback_router(update, context, data):
    if data == "colorlist_close":
        await update.callback_query.edit_message_text("پنل تغییر رنگ بسته شد.")
    elif data == "colorlist":
        await admin_button_color_list(update, context)
    elif data.startswith("colorpick:"):
        await admin_button_color_pick(update, context, int(data.split(":", 1)[1]))
    elif data.startswith("colorset:"):
        await admin_button_color_set(update, context, data.split(":", 1)[1])
    elif data.startswith("editplan_sel_"):
